...
async function loadFont(): Promise<ArrayBuffer> {
  // Try a nice bold display font; fall back to Inter if blocked
  try {
    const r = await fetch('https://fonts.cdnfonts.com/s/17808/Gilroy-ExtraBold.woff');
    if (r.ok) return await r.arrayBuffer();
  } catch {}
  const r2 = await fetch('https://fonts.gstatic.com/s/inter/v12/UcCO3FwrK3iLTeHuS_fvQtMwCp50KnMa1ZL7.woff2');
  return await r2.arrayBuffer();
}

export default async function handler(req: NextRequest, ctx: { params: { slug: string } }) {
